...
void moveCharacter() {
    ...
    for (int i = 0; i <= g_iPlatforms; i++)
    {
        if (g_skKeyEvent[K_57].keyDown && g_sUltimate == false) {
            if (g_sChar.m_cLocation.Y == (g_Console.getConsoleSize().Y - 1) || ((g_sChar.m_cLocation.Y + 1) == g_aPlatformsY[i] && (g_sChar.m_cLocation.X + 1) == g_aPlatformsX[i])) //Jump up
            {
                l = 0;
                l++;
            }
        ...
        if (l >= 1 && l < 4 && g_sUltimate == false) //check if player is mid air to move up to 4
        {
            if (g_jElapsedTime > 0.05)
            {
                ...
                if (g_sChar.m_cLocation.Y == g_aPlatformsY[x] && g_sChar.m_cLocation.X == g_aPlatformsX[i])
                {
                    g_sChar.m_cLocation.Y++;
                }
...
void downslam()
{
    ...
    if (g_sElapsedTime > 0.02)
    {
        ...
        else if (slam == 6)
        {
            ...
            {
                slam = 0;
                if (g_sChar.m_cLocation.X + 3 >= g_sEnemy.m_cLocation.X && g_sEnemy.m_cLocation.X > g_sChar.m_cLocation.X)
                {
...
void focusAttack()
{
    if (g_sFocus == true)
    {
        ...
        {
            su = 0;
            g_sChar.m_dMana = g_sChar.m_dMana - 10;
        }
...
void setdamage()
{
    if (g_sEnemy.m_cLocation.X == g_sChar.m_cLocation.X && g_sEnemy.m_cLocation.Y == g_sChar.m_cLocation.Y || g_sEnemy.m_cLocation.X == g_sChar.m_cLocation.X + 1 && g_sEnemy.m_cLocation.Y == g_sChar.m_cLocation.Y)
    {
        ...
        {
            if (g_sEnemy.m_dHealth > 0 && !g_sUltimate && !g_sRage && !g_sAttackState && !g_sInvulnerable && !g_sFocus)
            {
                g_sChar.m_dHealth--;
            }
            g_hElapsedTime = 0;
